...
export class EventpopPage {
  ...
  reg(name){
    ...
    this.http.get('http://localhost/capstone/php/regvent.php/a/'+this.a+'/none/'+name).subscribe(data => {
  ...
  ionViewDidLoad() {
    ...
    this.http.get('http://localhost/CAPSTONE/php/index.php/getevent/'+this.id).subscribe(data => {
      // Read the result field from the JSON response.
      //this.results = JSON.stringify(data);  
     // this.img.nativeElement.src= data[0]['ministryimage'];
     this.ministries = data;
